...
-- ARGV[1]=now(ms) ARGV[2]=eventId ARGV[3]=eventType
...
if eventType == 'SUBSCRIPTION_FAILED' then
    redis.call('ZADD', KEYS[1], 'NX', now, eventId)
    redis.call('EXPIRE', KEYS[1], ARGV[7])
end
redis.call('ZREMRANGEBYSCORE', KEYS[1], 0, now - tonumber(ARGV[4]))
local failCount = redis.call('ZCARD', KEYS[1])


-- 2. limit 윈도우 (SUBSCRIPTION_LIMIT_EXCEEDED)
if eventType == 'SUBSCRIPTION_LIMIT_EXCEEDED' then
    redis.call('ZADD', KEYS[2], 'NX', now, eventId)
    redis.call('EXPIRE', KEYS[2], ARGV[7])
end
redis.call('ZREMRANGEBYSCORE', KEYS[2], 0, now - tonumber(ARGV[5]))
local limitCount = redis.call('ZCARD', KEYS[2])

-- 3. recent(모든 타입) - NX로 중복 무시, 최근 sampleSize 개만 유지
local sampleSize = tonumber(ARGV[6])
redis.call('ZADD', KEYS[3], 'NX', now, eventId .. '|' .. eventType)
