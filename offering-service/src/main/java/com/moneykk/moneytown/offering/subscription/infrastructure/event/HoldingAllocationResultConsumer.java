...
public class HoldingAllocationResultConsumer {

    private static final TypeReference<
            EventEnvelope<HoldingAllocationSucceededPayload>
            > SUCCEEDED_TYPE = new TypeReference<>() {
    };

    private static final TypeReference<
            EventEnvelope<HoldingAllocationFailedPayload>
            > FAILED_TYPE = new TypeReference<>() {
    };

    private final ObjectMapper objectMapper;
    private final HoldingAllocationResultService holdingAllocationResultService;

    @KafkaListener(
            topics = "holding-allocation-result",
    ...
    public void consume(
        ...
        if (root == null || !root.isObject()) {
            throw new IllegalArgumentException(
                    "Holding 배정 결과 이벤트는 JSON 객체여야 합니다."
            );
        }
